...
// evaluate post fix exp
int postfixEvaluate(string post)
{
    stack<int> st;
    int n = post.size();
    ...
    for (int i = 0; i < n; i++)
    {
        // if operandis found pop 2 top of stack and apply the operand and store the result back in stack
        ...
        // else convert to int and store the number to int stack
        else
        {
            int no = post[i] - '0';
            st.push(no);
...
int main()
{
    string str = "((2+7)*3-2)/5";

    cout << "The postfix expression of " << str << " is ";
    string ans = infixToPostfix(str);

    cout << "\nAfter Evaluation of the expression answer is ";
    int exp = postfixEvaluate(ans);
    cout << exp << endl;
    return 0;
}